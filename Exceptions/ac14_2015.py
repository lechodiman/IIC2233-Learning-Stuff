...
class Calculadora:
    ...
    def leer_operaciones(self, declaracion):
        ...
        for operacion_actual in operaciones:
            try:
                ...
                print("[ERROR] ", str(err))
                print("Falta un operador en {}".format(declaracion))
            else:
        ...
        try:
            if operacion is Operaciones.suma:
                ...
                try:
                    ...
                    print("[ERROR] ", str(err))
                    print("1 dividido 0 es igual a infinito")
            else:
                resultado = operador1 % operador2
    ...
    def valor_en_numeros(self, numero_letra, declaracion):
        ...
        if numero_letra.isdigit():
            ...
            try:
                ...
                print("[ERROR] ", str(err))
                print("{} no tiene valores asignados".format(numero_letra))
            else:
                return v
    ...
    def agregar_letra(self, letra, valor):
        try:
            if letra in letras_especiales:
                raise KeyError
            else:
                self.letras_especiales[letra] = valor
        except KeyError as err:
            print("[ERROR] ", str(err))
            print("La letra 'g' no será agregada. Ya existe dentro de la calculadora.")
